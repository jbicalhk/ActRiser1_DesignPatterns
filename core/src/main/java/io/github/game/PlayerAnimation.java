...
public class PlayerAnimation {
	private static final int SPRITE_WIDTH = 96 / 6;
	...
	public PlayerAnimation(String spritePath) {
		...
		for (int i = 0; i < 6; i++) {
			frames[i] = new TextureRegion(fullSheet, i * SPRITE_WIDTH, 0, SPRITE_WIDTH, SPRITE_HEIGHT);
		}
	...
	private void setAnimation(PlayerState state, Direction direction) {
		...
		float frameDuration = state == PlayerState.MOVING ? 0.1f : 0.5f;
